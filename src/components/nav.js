...
const Nav = props => {
    ...
    function renderBtn () {
        if(props.auth) {
            return <button onClick={ () => props.switchAuthentication(false)} className="btn red darken-1">Sign Out</button>
        }

        return <button onClick={ () => props.switchAuthentication(true)} className="btn purple red-3">Sign In</button>
    }
    return (
